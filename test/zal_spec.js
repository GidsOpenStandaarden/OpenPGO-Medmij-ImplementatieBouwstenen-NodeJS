...
describe('Whitelist parser', function() {
  ...
  it('should create a whitelist parser from schema and XML files', function(done) {
    new ZAL(function(error, zal) {
      if (error) {
        ...
        expect(zal instanceof ZAL).toEqual(true);
        zalGlobal = zal;
        done();
      }
    })
  ...
  it('should process requests correctly', function() {
    expect(zalGlobal.getList()).toBeDefined();
    expect(zalGlobal.getZorgaanbieder('iets')).toEqual(null);
    expect(zalGlobal.getZorgaanbieder('umcharderwijk@medmij')).not.toBe(null);
  });
});